...
public class ResourceChecker : EditorWindow {
	...
	public class TextureDetails
	{
		...
		public TextureDetails(Texture texture)
		{
			this.texture = texture;
			isCubeMap = texture is Cubemap;

			memSizeBytes = CalculateTextureSizeBytes(texture);
			format = TextureFormat.RGBA32;
			mipMapCount = 1;
			if (texture is Texture2D)
			{
				format = ((Texture2D)texture).format;
				mipMapCount = ((Texture2D)texture).mipmapCount;
			}
			if (texture is Cubemap)
			{
				format = ((Cubemap)texture).format;
			}
		}
	}
...
//				return 0; //Not supported yet
	...
	void ListTextures()
	{
		...
		ActiveTextures.RemoveAll(t => !t.texture);
		...
		{			
			...
			if (tDetails.mipMapCount > 1)
			{
				sizeLabel+=" - "+tDetails.mipMapCount+" mipmap levels";
			}
			sizeLabel+="\n"+FormatSizeString(tDetails.memSizeBytes)+" - "+tDetails.format+"";
	...
	private static string[] FormatSizeSuffices = { "B", "KB", "MB", "GB" };
	public static string FormatSizeString(float size)
	{
		int order = 0;
		while (size >= 1024 && order + 1 < FormatSizeSuffices.Length) {
		    order++;
		    size = size/1024;
		}
		if (size < 10)
		{
			return string.Format("{0:0.##} {1}", size, FormatSizeSuffices[order]);
		}
		else if (size < 100)
		{
			return string.Format("{0:0.#} {1}", size, FormatSizeSuffices[order]);
	...
	void CheckResources()
	{
		...
		foreach (var texture in FindObjectsOfType<Texture>())
		{
			ActiveTextures.Add(new TextureDetails(texture));
		}
	...
	private TextureDetails GetTextureDetail(Texture tTexture, Material tMaterial, MaterialDetails tMaterialDetails)
	{
		...
		{
			tTextureDetails = new TextureDetails(tTexture);
		}
		if (tMaterial != null)
		{
